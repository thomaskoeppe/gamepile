 ...
 * @returns Array of game objects (including categories and tags) augmented with
 *   `playtime` (minutes), and an `owned` flag indicating whether
 *   the game is in the user's Steam library.
 */
export const getGamesForCollection = queryClientWithAuth.inputSchema(z.object({ collectionId: z.cuid() })).query<Array<Prisma.GameGetPayload<{ include: { categories: true, tags: true } }> & { playtime?: number; owned: boolean }>>(withLogging(async ({ parsedInput: { collectionId }, ctx }, { log })  => {
