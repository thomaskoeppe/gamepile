...
const workerEnvSchema = z.object({
    ...
    WORKER_ACHIEVEMENTS_CONCURRENCY: z.preprocess((value) => Number(value ?? 1), z.number().int().positive()),
    WORKER_ACHIEVEMENTS_BATCH_SIZE: z.preprocess((value) => Number(value ?? 10), z.number().int().min(1).max(50)),
